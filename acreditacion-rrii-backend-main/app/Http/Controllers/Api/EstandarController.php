...
class EstandarController extends Controller
{
    public function createEstandar(Request $request)
    {
        ...
        $estandar->dimension = $request->dimension;
        $estandar->factor = $request->factor;
        $estandar->estandarC = $request->estandarC;
        $estandar->save();
        return response([
            "status" => 1,
    ...
    public function updateEstandar(Request $request, $id)
    {
        ...
        if ($id_user->isEncargadoEstandar($id) || $id_user->isAdmin()) {
            ...
            $estandar->dimension = isset($request->dimension) ? $request->dimension : $estandar->dimension;
            $estandar->factor = isset($request->factor) ? $request->factor : $estandar->factor;
            $estandar->estandarC = isset($request->estandarC) ? $request->estandarC : $estandar->estandarC;
